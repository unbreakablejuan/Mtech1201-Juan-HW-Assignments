...
function draw() {
  ...
  for (let i = squares.length - 1; i >= 0; i--) {
   ...
   // rect(mouseX, 0, s.size / 4, s.size / 16,s.size,s.size,s.size,s.size);
    //rect(0, mouseY, s.size / 16, s.size / 4,s.size,s.size,s.size,s.size);
    rect(0, mouseX, s.size / 4, s.size / 16,s.size,s.size,s.size,s.size);
    rect(mouseY, 0, s.size / 16, s.size / 4,s.size,s.size,s.size,s.size);
    pop();
